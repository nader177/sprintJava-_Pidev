 ...
 */
public class Pdf {
    public void orderPdf(commandes c) {
        try {
            ...
            PdfWriter.getInstance(document, new FileOutputStream("C:\\xampp\\htdocs\\img\\" + c.getId() + "_" + dateString + ".pdf"));
            ...
            Chunk chunk = new Chunk("  commandes ", fontgrastitre);
            document.add(chunk);
            document.add(new Paragraph("\n\n"));
            document.add(new Paragraph("\n\n"));
            chunk = new Chunk("  Faite le :  " + c.getDate(), fontgras);
            document.add(chunk);
            Image image = Image.getInstance("C:\\xampp\\htdocs\\img\\");
            image.scaleToFit(100, 100);
            image.setAbsolutePosition(460, 720);
            document.add(image);
            document.add(new Paragraph("\n\n"));
            chunk = new Chunk(" Montant:  " + c.getNom()+ " DT", fontgras);
            document.add(chunk);
            document.add(new Paragraph("\n"));
            chunk = new Chunk(" Request From: "+c.getPrenom(), fontgras);
            document.add(chunk);
                       
            document.add(new Paragraph("\n"));
            chunk = new Chunk(" Request To: "+ c.getAdress(), fontgras);
            document.add(chunk);
            document.add(new Paragraph("\n"));
            chunk = new Chunk("  Agnece  :  " + c.getDate(), fontgras);
            document.add(chunk);
            document.add(new Paragraph("\n"));
            chunk = new Chunk(" Type  :  "+c.getTel(), fontgras);           
            document.add(new Paragraph("\n"));
            document.add(chunk);
